...
@Injectable()
export class CodexSemanticService {
  ...
  async regenerateReply(input: RegenerateReplyInput): Promise<AnalyzeReviewOutput> {
    const review = AnalyzeReviewInputSchema.parse(input.review);
    return this.runCodex(buildRegeneratePrompt({ ...input, review }));
  }
...
async function runProcess(
  ...
  await new Promise<void>((resolveRun, reject) => {
    ...
    child.on("close", (code) => {
      ...
      reject(new Error(`codex exec failed with exit code ${code}: ${Buffer.concat(stderr).toString("utf8")}`));
    });
  });
}
